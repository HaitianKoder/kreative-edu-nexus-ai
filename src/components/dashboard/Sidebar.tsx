    ...
    <div className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 bg-gray-50 border-r border-gray-200">
      <div className="flex-1 flex flex-col pt-5">
        <div className="flex items-center flex-shrink-0 px-4">
          ...
          <span className="text-lg font-semibold text-gray-900">CampusAI</span>
        </div>
          ...
          {menuItems.map((section, index) => (
            <div key={index} className="space-y-2">
              ...
              {section.items.map((item) => (
                ...
                >
                  <item.icon
                    className={`mr-3 h-6 w-6 ${location.pathname === item.href ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'
                      }`}
                    aria-hidden="true"
                  />
                  {item.name}
      ...
      <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
        <a href="#" className="flex-shrink-0 group block">
          <div className="flex items-center">
            <div>
              {/* Assuming you have an Avatar component or similar */}
